...
int main(void) {
    ...
    while((int)winners.size() > 2){
        vector<int> nxt;
        int m = winners.size()/2;
        rep(i,m) {
            if(a[winners[i*2]] > a[winners[i*2+1]]) {
                nxt.push_back(winners[i*2]);
            } else {
                nxt.push_back(winners[i*2+1]);
            }
        }
        winners = move(nxt);
    }
    if (a[winners[0]] > a[winners[1]]) {
        cout << winners[1] + 1 << endl;
    } else {
        cout << winners[0] + 1 << endl;
    }
    return 0;
}
