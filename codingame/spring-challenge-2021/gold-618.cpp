...
struct Action {
    ...
    void calc_weight() {
        ...
        if (planned_kind != kind) weight = 0;
            ...
            int disturb = calc_disturb(cellIndex, 3);
            assert(planned_kind[5] - '0' == tree_size);
            if (true) weight = cells[cellIndex].richness;
            else
            if (kind == "GROW 2 -> 3") {
                weight = 10 - disturb;
            }
            else if (kind == "GROW 1 -> 2") {
                weight = 10 - disturb;
            }
            else if (kind == "GROW 0 -> 1") {
                weight = 10 - disturb;
            }
            else {
                assert(false);
...
void plan_daily_schedule() {
    ...
    auto dfs = [&](auto &&f, int depth, deque<string> path, int rem_sun) -> void{
        if (depth == 0) {
            ...
            // int forecast_score = score;
            // int forecast_nutrients = nutrients;
            while (rem_sun >= 4 && size3 > 0) {
                // Heuristic
                if (day < 20 && size3 < 5) return;
                // if (day < 23 && forecast_score >= 100) return; 

                path.push_back("COMPLETE");
                rem_sun -= 4;
                size3--;
                // forecast_score += forecast_nutrients;
                // forecast_nutrients--;
                f(f, depth+1, path, rem_sun);
            }
        }
