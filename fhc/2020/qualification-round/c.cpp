...
void solve(){
    ...
    for (auto itr: stand) {
        ll p = itr.first;
        ll h = itr.second;
        right[p+h] = max(right[p+h], h + right[p]);
        res = max(res, right[p+h]);
    }
    for (auto itr = stand.rbegin(); itr != stand.rend(); ++itr) {
        ll p = itr->first;
        ll h = itr->second;
        left[p-h] = max(left[p-h], h + left[p]);
        res = max(res, right[p-h] + left[p-h]);
    }
    cout << res << endl;
}
